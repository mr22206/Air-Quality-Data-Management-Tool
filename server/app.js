...
const app = express();
const __dirname = path.resolve(); // Ajout de la variable __dirname
app.use(express.static(path.join(__dirname, './'))); // Utilisation de path.join pour définir le chemin absolu
const corsOptions = {
  origin: ['http://localhost:5173', 'https://rwrz.ddns.net', 'http://localhost:3000'], // Les domaines autorisés à accéder à votre serveur
  methods: ['GET', 'POST'], // Les méthodes HTTP autorisées
  allowedHeaders: ['Content-Type', 'Authorization'], // Les en-têtes autorisés
  credentials: true, // Indique si les en-têtes CORS doivent être inclus dans les réponses
};
